...
> **必须先运行脚本、拿到 JSON，再撰写**。脚本跑出数据前，**禁止**凭训练知识 / 记忆 / 网页搜索自行编写任何周报内容；
...
- **前置检查**：确认第 1 步脚本已在本次会话跑过并产出 `/tmp/td-weekly-merged.json`；没有就先回到第 1 步，**绝不跳过、绝不凭记忆/训练知识/网页搜索代替**。
...
- **商业化机会分析**：周报须含「💡 商业化机会分析（基于社区痛点）」段，从本周真实社区原话里的未满足需求提炼 2-4 条机会（锚定原话 + `url`、标注「推断」、优先点出中外差异/国产替代机会），绝不虚构需求或市场数据。详见 competitor-monitor.md 对应段。
- **输出报告文件**：写完后按 `<ENGINE_DIR>/follow-news-addons/references/templates/feishu.md` 规范，
  把最终报告另存为 `reports/ai-pulse-weekly-<YYYY-MM-DD>.md`（飞书云文档兼容，先 `mkdir -p reports`），并把文件路径告知用户。
