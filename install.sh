...
# ─── Step 5b: 品牌字符串重写（template / digest-prompt 的 footer）───
# patch 不覆盖这几个文件，单独 sed 以避免 patch hunk 上下文脆弱。
# 影响：用户看到的报告 footer 显示 "Generated by ai-pulse v<VERSION>" 而非旧的 follow-news 字串。
FOOTER_FILES=(
    "$FOLLOW_NEWS_DIR/references/templates/chat.md"
    "$FOLLOW_NEWS_DIR/references/templates/discord.md"
    "$FOLLOW_NEWS_DIR/references/templates/email.md"
    "$FOLLOW_NEWS_DIR/references/templates/pdf.md"
    "$FOLLOW_NEWS_DIR/references/digest-prompt.md"
)
for f in "${FOOTER_FILES[@]}"; do
    if [ -f "$f" ]; then
        # 仅替换 footer 行的品牌字串；archive 路径、config 文件名等不动（防止破坏现有工作流）
        sed -i.bak \
            -e 's|Generated by follow-news v3\.9\.1|Generated by ai-pulse v<VERSION>|g' \
            -e 's|Generated by follow-news|Generated by ai-pulse|g' \
            -e 's|>follow-news</a>|>ai-pulse</a>|g' \
            -e 's|github\.com/tangwz/follow-news|github.com/huudage/ai-pulse|g' \
            "$f" && rm -f "${f}.bak"
    fi
done
echo "  ✓ footer 品牌字串改写为 ai-pulse"
