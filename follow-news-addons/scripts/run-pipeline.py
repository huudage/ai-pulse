...
Runs all fetch steps (RSS, Twitter, GitHub, GitHub Trending, Web, Podcast) in parallel,
...
def main() -> int:
    ...
    parser.add_argument("--skip", type=str, default="", help="Comma-separated list of steps to skip (rss,twitter,github,trending,web,podcast)")
    parser.add_argument("--only", type=str, default="", help="Comma-separated list of steps to run (rss,twitter,github,trending,web,podcast). Others are skipped.")
    ...
    parser.add_argument("--no-archive-json", dest="archive_json", action="store_false", default=True, help="Skip archiving the final merged JSON into <archive-dir>/daily-json/<date>.json (default: archive)")
    ...
    if only_steps:
        all_step_keys = {"rss", "twitter", "github", "trending", "web", "podcast"}
    ...
    for flag, path in [("--rss", tmp_rss), ("--twitter", tmp_twitter),
                       ("--github", tmp_github), ("--trending", tmp_trending),
    ...
    health_args = []
    for flag, path in [("--rss", tmp_rss), ("--twitter", tmp_twitter), ("--github", tmp_github), ("--web", tmp_web), ("--podcast", tmp_podcast)]:
        if path.exists():
