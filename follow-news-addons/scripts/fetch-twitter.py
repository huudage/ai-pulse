...
OPENCLI_BROWSER_SESSION_DEFAULT = "follow-news"
...
def get_opencli_browser_session() -> str:
    """OpenCLI 1.8.0+ requires every `browser <session> ...` call to carry a session
    name. Reusing the same name keeps the tab/state alive across invocations."""
    raw_value = os.getenv("OPENCLI_BROWSER_SESSION", "").strip()
    return raw_value or OPENCLI_BROWSER_SESSION_DEFAULT
...
class OpenCliBackend(TwitterBackend):
    ...
    def _list_browser_tabs(self) -> Optional[Dict[str, str]]:
        ...
        result = self._run_command(["browser", get_opencli_browser_session(), "tab", "list"], timeout=OPENCLI_TAB_COMMAND_TIMEOUT)
        if result.returncode != 0:
    ...
    def _cleanup_new_browser_tabs(self, before_tabs: Optional[Dict[str, str]]) -> None:
        ...
        for target_id in pending_targets:
            result = self._run_command(
                ["browser", get_opencli_browser_session(), "tab", "close", target_id],
                timeout=OPENCLI_TAB_COMMAND_TIMEOUT,
            )
            if result.returncode != 0:
                logging.warning(
    ...
    def _release_browser_lease(self) -> None:
        ...
        result = self._run_command(["browser", get_opencli_browser_session(), "close"], timeout=OPENCLI_TAB_COMMAND_TIMEOUT)
        if result.returncode != 0:
